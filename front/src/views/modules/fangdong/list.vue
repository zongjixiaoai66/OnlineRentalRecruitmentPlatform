        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '房东姓名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.fangdongName" placeholder="房东姓名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('fangdong','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/zaixianzufangzhaopinoingtai/upload/fangdongMuBan.xls"
                        >批量导入房东数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('fangdong','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="fangdongUploadSuccess"
                                :on-error="fangdongUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('fangdong','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "fangdong.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="fangdongIdNumber"
                                   header-align="center"
                                   label="房东身份证号">
                        <template slot-scope="scope">
                            {{scope.row.fangdongIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="fangdongPhoto"
                                     header-align="center"
                                     width="200"
                                     label="房东头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.fangdongPhoto">
                                <img :src="$base.url+scope.row.fangdongPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('fangdong','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('fangdong','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('fangdong','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('fangdong','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'fangdong',//当前表表名,
                        sumColum : 'fangdong_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'fangdong_name',//分组字符串字段
                        // types : 'fangdong_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['fangdongDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "fangdong/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            fangdongUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "fangdong/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
